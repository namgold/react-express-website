...
const T = {
  ...
  setCookie: (cname, cvalue, exdays) => {
    const d = new Date();
    d.setTime(
      d.getTime() + (exdays === undefined ? 60 : exdays) * 24 * 60 * 60 * 1000,
    );
  ...
  validateEmail: (email) => /^(([^<>()[\].,;:\s@"]+(\.[^<>()[\].,;:\s@"]+)*)|(".+"))@(([^<>()[\].,;:\s@"]+\.)+[^<>()[\].,;:\s@"]{2,})$/i.test(
    String(email).toLowerCase(),
  ),

  dateToText: (date, format) => dateformat(date, format || 'dd/mm/yyyy HH:MM:ss'),
  ...
  notify: (message, type) => $.notify(
    { message },
    { type, placement: { from: 'bottom' }, z_index: 2000 },
  ),
...
['get', 'post', 'put', 'delete'].forEach(
  (method) => {
    T[method] = (url, data, success, error) => {
      ...
      $.ajax({
        ...
        success: (_data) => success && success(_data),
        error: (_data) => {
          console.error(
            `Ajax (${method.toUpperCase()} => ${url}) has error. Error:`,
            data,
          );
          if (error) error(_data.responseJSON);
        },
      });
    };
  },
...
Date.prototype.getText = function () {
  return `${T.language.getMonth()[this.getMonth()]} ${T.get2(
    this.getDate(),
  )}, ${this.getFullYear()} ${T.get2(this.getHours())}:${T.get2(
    this.getMinutes(),
  )}`;
};
...
Date.prototype.strNgay = function () {
  return `${T.get2(this.getDate())}/${T.get2(
    this.getMonth(),
  )}/${this.getFullYear()}`;
};
...
Date.prototype.getShortText = function () {
  return `${this.getFullYear()}/${T.get2(this.getMonth() + 1)}/${T.get2(
    this.getDate(),
  )} ${T.get2(this.getHours())}:${T.get2(this.getMinutes())}`;
};
Date.prototype.ddmmyyyy = function () {
  return this.getDate()
    ? `${T.get2(this.getDate())}/${T.get2(
      this.getMonth() + 1,
    )}/${this.getFullYear()}`
    : '';
};
Date.prototype.mmyyyy = function () {
  return this.getDate()
    ? `${T.get2(this.getMonth() + 1)}/${this.getFullYear()}`
    : '';
};
